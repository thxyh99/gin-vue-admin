...
// ImportExcel 导入表格
func (wcStaffSocialApi *WcStaffSocialApi) ImportExcel(c *gin.Context) {
	...
	if err := wcStaffSocialService.ImportExcel(templateID, socialType, file); err != nil {
...
func (wcStaffSocialApi *WcStaffSocialApi) ExportExcel(c *gin.Context) {
	...
	if file, name, err := wcStaffSocialService.ExportExcel(templateID, queryParams); err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
	} else {
