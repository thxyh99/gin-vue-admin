<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="绩效月份" prop="month" width="120" />
        <el-table-column align="left" label="绩效金额" prop="amount" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateAchTestFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="绩效月份">
                        {{ formData.month }}
                </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getAchTestList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteAchTestByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateAchTestFunc = async(row) => {
    const res = await findAchTest({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteAchTestFunc = async (row) => {
    const res = await deleteAchTest({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
