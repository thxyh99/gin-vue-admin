<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="职级类型" prop="rankType" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWcSalaryTemplateFunc(scope.row)">变更</el-button>
       ...
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="职级类型:"  prop="rankType" >
              <el-input v-model.number="formData.rankType" :clearable="true" placeholder="请输入职级类型" />
            </el-form-item>
          </el-form>
    </el-drawer>
...
import {
  ...
  deleteWcSalaryTemplate,
  deleteWcSalaryTemplateByIds,
  updateWcSalaryTemplate,
  findWcSalaryTemplate,
  getWcSalaryTemplateList
...
const getTableData = async() => {
  const table = await getWcSalaryTemplateList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteWcSalaryTemplateFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteWcSalaryTemplateByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWcSalaryTemplateFunc = async(row) => {
    const res = await findWcSalaryTemplate({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWcSalaryTemplateFunc = async (row) => {
    const res = await deleteWcSalaryTemplate({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
