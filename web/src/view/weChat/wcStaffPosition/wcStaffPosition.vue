  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWcStaffPositionFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteWcStaffPosition,
  deleteWcStaffPositionByIds,
  updateWcStaffPosition,
  findWcStaffPosition,
  getWcStaffPositionList
...
const getTableData = async() => {
  const table = await getWcStaffPositionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteWcStaffPositionByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWcStaffPositionFunc = async(row) => {
    const res = await findWcStaffPosition({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWcStaffPositionFunc = async (row) => {
    const res = await deleteWcStaffPosition({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
