  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWcStaffMaterialsFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteWcStaffMaterials,
  deleteWcStaffMaterialsByIds,
  updateWcStaffMaterials,
  findWcStaffMaterials,
  getWcStaffMaterialsList
...
const getTableData = async() => {
  const table = await getWcStaffMaterialsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteWcStaffMaterialsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteWcStaffMaterialsByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 删除行
const deleteWcStaffMaterialsFunc = async (row) => {
    const res = await deleteWcStaffMaterials({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
