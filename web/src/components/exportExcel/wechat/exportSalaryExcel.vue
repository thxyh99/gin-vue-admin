...
const exportExcelFunc = async() => {
  ...
  const params = Object.entries(paramsCopy)
    ...
    .join('&')
  const url = `${baseUrl}/wcStaffSalary/exportExcel?templateID=${props.templateId}&type=${props.type}&staffId=${props.staffId}&month=${props.month}&keyword=${props.keyword}${params ? '&' + params : ''}`
